...
# --- CUSTOM STYLES (light mode + blue palette) ---
st.markdown("""
    <style>
    body {
...
# --- PAGE HEADER ---
st.markdown('<div class="top-right">Created by Loïc Persyn with the help of AI</div>', unsafe_allow_html=True)
...
with st.expander("🗂 Step 1: Upload Files", expanded=True):
    ...
    # Show total count of uploaded files
    if uploaded_files:
...
if st.session_state.uploaded_files:
    with st.expander("⚙️ Step 2: Extract & Validate", expanded=True):
        st.markdown('<div class="blue-container">', unsafe_allow_html=True)
    ...
    # Instructions when no upload yet
    st.markdown("---")
